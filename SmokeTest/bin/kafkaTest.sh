...
rc=$?; if [[ $rc != 0 ]]; then echo "Can not create Topic! exiting";  echo " - Kafka	- Failed [Can not create Topic]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
rc=$?;echo "exitcode: "$rc; if [[ ($rc != 0) && ($rc != 130) ]]; then echo "Can not produce data! exiting";  echo " - Kafka	- Failed [Can not produce data]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ ($rc != 0) && ($rc != 130) ]]; then echo "Can not consume data! exiting";  echo " - Kafka	- Failed [Can not consume data]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
then
	...
	echo "******************************************"
	echo " - Kafka	- Passed " >> ./log/${timestamp}SummaryReport.txt
else
	echo "Not same data as produced"
	echo "******************************************"
	echo "* Kafka test completed Successfully! *"
	echo "******************************************"
	echo " - Kafka	- Failed[not Consuming produced data] " >> ./log/${timestamp}SummaryReport.txt
fi
