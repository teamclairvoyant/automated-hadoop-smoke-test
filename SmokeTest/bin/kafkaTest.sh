...
$KAFKA_HOME/kafka-topics  --zookeeper ${ZOOKEEPER} --create --topic ${TOPIC_NAME} --partitions 1 --replication-factor 1
...
$KAFKA_HOME/kafka-console-producer --broker-list ${KAFKA_HOST} --topic ${TOPIC_NAME} < $KAFKA_INP_LOC
...
$KAFKA_HOME/kafka-console-consumer   --bootstrap-server ${KAFKA_HOST} --topic ${TOPIC_NAME} --from-beginning > $KAFKA_OUP_LOC
