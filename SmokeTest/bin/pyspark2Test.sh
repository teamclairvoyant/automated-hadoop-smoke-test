...
rc=$?; if [[ $rc != 0 ]]; then echo "Cannot produce input data! exiting"; echo " - pySpark2     - Failed [Cannot produce input data]" >> ./log/SummaryReport.txt; exit $rc; fi

hdfs dfs -put -f pyspark2_test.txt "$PYSPARK_IN_CLUS"
rc=$?; if [[ $rc != 0 ]]; then echo "Cannot copy input data! exiting"; echo " - pySpark2     - Failed [Cannot copy input data]" >> ./log/SummaryReport.txt; exit $rc; fi
