...
rc=$?; if [[ $rc != 0 ]]; then echo "Invalidation failed! exiting"; echo " - Impala	- Failed [Invalidation failed]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi

impala-shell -i  $IMPALADAEMON -q "select * FROM test_impala;" | tail -n +3 | sed -r 's/[-|+]+/ /g' | awk '{$1=$1};1' > $IMPALA_VAL
rc=$?; if [[ $rc != 0 ]]; then echo "Select query failed! exiting"; echo " - Impala	- Failed [Select query failed]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
if [[ $status = 0 ]]; then
	...
	echo "***************************************"
	echo " - Impala	- Passed" >> ./log/${timestamp}SummaryReport.txt
else
	echo "Files are different"
	echo "***************************************"
	echo "* Impala test not  completed Successfully! *"
	echo "***************************************"
	echo " - Impala	- Failed[datas in impala and date inserted are different]" >> ./log/${timestamp}SummaryReport.txt
fi
