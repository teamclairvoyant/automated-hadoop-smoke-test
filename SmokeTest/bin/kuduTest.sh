...
if $KUDU_SSL_ENABLED; then
	echo "IKOPTS: $IKOPTS"
	echo "ITOPTS: $ITOPTS"
	KUDU_IMPALA_CONNECT_STRING="${KUDU_IMPALA_CONNECT_STRING} ${IKOPTS} ${ITOPTS}"
fi

impala-shell -i "$KUDU_IMPALA_CONNECT_STRING" -q 'CREATE TABLE ${KUDU_TABLE_NAME}(id BIGINT, name STRING, PRIMARY KEY(id)) PARTITION BY HASH PARTITIONS 3 STORED AS KUDU;'
rc=$?; if [[ $rc != 0 ]]; then echo "Create query failed! exiting"; echo " - Kudu 		- Failed [Create query failed]" >> ./log/SummaryReport.txt; exit $rc; fi

impala-shell -i "$KUDU_IMPALA_CONNECT_STRING" -q 'INSERT INTO TABLE ${KUDU_TABLE_NAME} VALUES (1, "wasim"), (2, "ninad"), (3, "mohsin");'
...
impala-shell -i "$KUDU_IMPALA_CONNECT_STRING" -q 'SELECT * FROM ${KUDU_TABLE_NAME} WHERE id=1;'
