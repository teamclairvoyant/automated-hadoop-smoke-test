...
printf "create %s, 'cf'" "$HBASE_TABLE_NAME" | hbase shell 2>&1 | grep -q "Hbase::Table - ${HBASE_TABLE_NAME}" 2>/dev/null
...
printf "list %s" "$HBASE_TABLE_NAME" | hbase shell 2>&1
printf "list %s" "$HBASE_TABLE_NAME" | hbase shell 2>&1 | grep -q "[\"${HBASE_TABLE_NAME}\"]" 2>/dev/null
rc=$?; if [[ $rc != 0 ]]; then echo "List command failed! exiting"; echo " - HBase	- Failed [List command failed]" >> ./log/SummaryReport.txt; exit $rc; fi

printf "put %s, 'row1', 'cf:a', 'value1'" "$HBASE_TABLE_NAME" | hbase shell 2>&1 |grep -q "0 row(s) in" 2>/dev/null
rc=$?; if [[ $rc != 0 ]]; then echo "Put command failed! exiting"; echo " - HBase	- Failed [Put command failed]" >> ./log/SummaryReport.txt; exit $rc; fi

printf "scan %s" "$HBASE_TABLE_NAME" | hbase shell 2>&1
printf "scan %s" "$HBASE_TABLE_NAME" | hbase shell 2>&1 |grep -q "1 row(s) in" 2>/dev/null
