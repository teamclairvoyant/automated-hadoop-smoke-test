...
if $SECURITY_HIVE ; then
	...
	beeline -n `whoami` -u "jdbc:hive2://${HIVESERVER2}/${BKOPTS}${BTOPTS}" -e "CREATE TABLE test(id INT, name STRING) ROW FORMAT DELIMITED FIELDSTERMINATED BY ' ' STORED AS TEXTFILE;"
	rc=$?; if [[ $rc != 0 ]]; then echo "Create query failed! exiting"; echo " - Hive	- Failed [Create query failed]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
	echo "1 justin" > $HIVE_DATA_PATH
	rc=$?; if [[ $rc != 0 ]]; then echo "Input data generation failed! exiting"; echo " - Hive		- Failed [Input data generation failed]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
	echo "2 michael" >> $HIVE_DATA_PATH
	rc=$?; if [[ $rc != 0 ]]; then echo "Input data generation failed! exiting"; echo " - Hive		- Failed [Input data generation failed]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
	hdfs dfs -put $HIVE_DATA_PATH  $HIVE_TABLE_LOC
	rc=$?; if [[ $rc != 0 ]]; then echo "Input data transfer failed! exiting"; echo " - Hive		- Failed [Input data transfer failed]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
	beeline -n `whoami` -u "jdbc:hive2://${HIVESERVER2}/${BKOPTS}${BTOPTS}" -e"SELECT * FROM test WHERE id=1;"
	rc=$?; if [[ $rc != 0 ]]; then echo "Select query failed! exiting"; echo " - Hive		- Failed [Select query failed]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
