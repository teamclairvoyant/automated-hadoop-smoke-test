...
mainProcessID=$(curl -s $NIFI_BASE_API_URL/flow/process-groups/root/status?recursive=true |
    python -c "import sys, json; print (json.load(sys.stdin)['processGroupStatus']['id'])")

curl -X POST -H 'Content-Type: application/json' -d '{"revision":{"version":0},"component":{"name":"SmokeTestGroup","position":{"x":0,"y":0}}}' $NIFI_BASE_API_URL/process-groups/"$mainProcessID"/process-groups
...
# Template Upload
curl -k -F template=@SmokeTest.xml -X POST $$NIFI_BASE_API_URL/process-groups/"$processID"/templates/upload
...
curl -H 'Content-Type:application/json' -d '{"originX":"0","originY":"0","templateId":"'"$templateID"'"}' -X POST $NIFI_BASE_API_URL/process-groups/"$processID"/template-instance
...
# Start Process
curl -i -X PUT -H 'Content-Type: application/json' -d '{"state":"RUNNING","id":"'"$processID"'"}' $NIFI_BASE_API_URL/flow/process-groups/"$processID"
...
# Stop Process
curl -i -X PUT -H 'Content-Type: application/json' -d '{"state":"STOPPED","id":"'"$processID"'"}' $NIFI_BASE_API_URL/flow/process-groups/"$processID"
...
compID=$(curl -X GET $NIFI_BASE_API_URL/process-groups/"$processID"/connections |
    python -c "import sys, json; print (json.load(sys.stdin)['connections'][0]['component']['id'])")

compVersion=$(curl -X GET $NIFI_BASE_API_URL/process-groups/"$processID"/connections |
    python -c "import sys, json; print (json.load(sys.stdin)['connections'][0]['revision']['version'])")

curl -i -X POST $NIFI_BASE_API_URL/flowfile-queues/"$compID"/drop-requests
...
curl -i -X DELETE $NIFI_BASE_API_URL/connections/"$compID"?version="$compVersion"
...
# Delete template
curl -X DELETE $NIFI_BASE_API_URL/templates/"$templateID"
...
curl -X DELETE $NIFI_BASE_API_URL/process-groups/"$processID"?version="$processVersion"
