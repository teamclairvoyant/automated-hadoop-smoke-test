...
EOF
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to create temporary Nifi template! Exiting!";  echo " - Nifi	- Failed [Unable to instantiate template]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to create new process group! Exiting!";  echo " - Nifi	- Failed [Unable to instantiate template]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to upload template to Nifi! Exiting!";  echo " - Nifi	- Failed [Unable to upload template to Nifi]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to instantiate template! Exiting!";  echo " - Nifi	- Failed [Unable to instantiate template]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to start the process! Exiting!";  echo " - Nifi	- Failed [Unable to start the process]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to stop the process! Exiting!";  echo " - Nifi	- Failed [Unable to stop the process]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Failed to post queue drop request! Exiting!";  echo " - Nifi	- Failed [Failed to post drop request]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi

sleep 5s

curl -i -X DELETE http://{$NIFI_HOST}/nifi-api/connections/{$compID}?version={$compVersion}
rc=$?; if [[ $rc != 0 ]]; then echo "Connection couldn't be dropped! Exiting!";  echo " - Nifi	- Failed [Connection couldn't be dropped]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi

# Delete template
curl -X DELETE http://{$NIFI_HOST}/nifi-api/templates/{$templateID}
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to stop the process! Exiting!";  echo " - Nifi	- Failed [Template couldn't be deleted from Nifi]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Process group couldn't be deleted! Exiting!";  echo " - Nifi	- Failed [Process group couldn't be deleted]" >> ./log/${timestamp}SummaryReport.txt; exit $rc; fi

echo "***************************************"
echo "*  Nifi test completed Successfully!  *"
echo "***************************************"

echo " - Nifi		- Passed" >> ./log/${timestamp}SummaryReport.txt